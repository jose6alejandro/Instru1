...
vc = importdata('Medidas/critico_capacitor/VOLTAJE.CSV',',',18);
voltaje_Data =  vc.data(:, 4:5);

ic = importdata('Medidas/critico_capacitor/CORRIENTE.CSV',',',18);
corriente_Data =  ic.data(:, 4:5);

il = importdata('Medidas/critico_inductor/CORRIENTE.CSV',',',18);
corriente2_Data =  il.data(:, 4:5);

vl = importdata('Medidas/critico_inductor/VOLTAJE.CSV',',',18);
voltaje2_Data =  vl.data(:, 4:5);

% Capacitor
figure(1)
subplot(2, 2, 1);
plot(tiempo_total,valores_ode_total(:,1),'g', tiempo_total, VC_maple_total,'r', voltaje_Data(:, 1) + 0.01465, voltaje_Data(:, 2) - 0.073,'b');
title('Voltaje del capacitor (Critico amortiguado)')
xlabel('Tiempo (seg)')
ylabel('Voltaje (V)')
...
title('Corriente del capacitor (Critico amortiguado)')
xlabel('Tiempo (seg)')
ylabel('Corriente (A)')
legend('ODE','Maple','Experimental')
grid on;
...
subplot(2, 2, 4);
plot(tiempo_total, voltaje_inductor, 'g', tiempo_total, voltaje_inductor_maple, 'r', voltaje2_Data(:, 1) + 0.015, voltaje2_Data(:, 2),'b');
title('Voltaje del inductor (Critico amortiguado)')
xlabel('Tiempo (seg)')
ylabel('Voltaje (V)')
legend('ODE','Maple','Experimental')
grid on;
