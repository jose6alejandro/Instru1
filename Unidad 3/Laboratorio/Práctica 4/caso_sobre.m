...
vc = importdata('Medidas/sobre_capacitor/VOLTAJE.CSV',',',18);
voltaje_Data =  vc.data(:, 4:5);

ic = importdata('Medidas/sobre_capacitor/CORRIENTE.CSV',',',18);
corriente_Data =  ic.data(:, 4:5);

il = importdata('Medidas/sobre_inductor/CORRIENTE.CSV',',',18);
corriente2_Data =  il.data(:, 4:5);

vl = importdata('Medidas/sobre_inductor/VOLTAJE.CSV',',',18);
...
subplot(2, 2, 2);
plot(tiempo_total, valores_ode_total(:,3),'g', tiempo_total, IC_maple_total,'r', corriente_Data(:, 1)  + 0.01465, corriente_Data(:, 2)/R3 + 6.6668099e-4,'b');
title('Corriente del capacitor (sobreamortiguado)')
xlabel('Tiempo (seg)')
ylabel('Corriente (A)')
legend('ODE','Maple','Experimental')
grid on;
...
subplot(2, 2, 3);
plot(tiempo_total, corriente_inductor, 'g', tiempo_total, IL_maple_total, 'r', corriente2_Data(:, 1) + 0.0149, corriente2_Data(:, 2)/(R2-RL) + 7e-05,'b');
title('Corriente del inductor (sobreamortiguado)')
xlabel('Tiempo (seg)')
ylabel('Corriente (A)')
legend('ODE','Maple','Experimental')
grid on;
...
subplot(2, 2, 4);
plot(tiempo_total, voltaje_inductor, 'g', tiempo_total, voltaje_inductor_maple, 'r', voltaje2_Data(:, 1) + 0.0149, voltaje2_Data(:, 2),'b');
title('Voltaje del inductor (sobreamortiguado)')
xlabel('Tiempo (seg)')
ylabel('Voltaje (V)')
legend('ODE','Maple','Experimental')
grid on;
...
% IC
tiempo = corriente_Data(:, 1) + 0.01465;
...
% IL
tiempo = corriente2_Data(:, 1) + 0.0149;
corriente = corriente2_Data(:, 2)/(R2-RL) + 7e-05;
...
longitud = length(tt);
errorILmape = sum(abs(((ilData(43:433,2)+5) - (IL_maple_ + 5)) ./ (ilData(43:433,2)+5))) * 100 / longitud;
%figure(3)
%plot(tt, IL_maple_, tt, ilData(43:433,2));

% VL
tiempo = voltaje2_Data(:, 1) + 0.0149;
...
errorVLmape = sum(abs(((vlData(50:433,2)+5) - (VL_maple_total + 5)) ./ (vlData(50:433,2)+5))) * 100 / longitud;
%figure(3)
